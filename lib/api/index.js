...
module.exports = {
  ...
  startConcurrentSpan(metadataContext) {
    return apiImpl.startConcurrentSpan(metadataContext);
  },
  finishConcurrentSpan(span, rollupKey) {
    return apiImpl.finishConcurrentSpan(span, rollupKey);
  },
  ...
  withConcurrentSpan(metadataContext, fn, rollupKey) {
    const span = apiImpl.startConcurrentSpan(metadataContext);
    try {
      return fn();
    } finally {
      apiImpl.finishConcurrentSpan(span, rollupKey);
    }
  },
  withSpanAsync(metadataContext, fn, rollupKey) {
    const span = apiImpl.startSpan(metadataContext);
    return fn().then(
      args => {
        apiImpl.finishSpan(span), rollupKey;
  ...
  withConcurrentSpanAsync(metadataContext, fn, rollupKey) {
    const span = apiImpl.startConcurrentSpan(metadataContext);
    return fn().then(
      args => {
        apiImpl.finishConcurrentSpan(span), rollupKey;
